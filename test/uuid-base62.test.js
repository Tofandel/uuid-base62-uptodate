...
describe('uuid-base62', function () {
    ...
    describe('other bases', function () {
        it('should accept a custom base as a general option', function () {
        ...
        it('should accept a custom base as an option', function () {
            var customBase = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-_";
            var uuid = '72be7291-fbf6-400f-87c4-455e23d01cd5';

            var uuidB64 = uuidBase62.encode(uuid, {base: customBase});
            assert.equal(uuidB64, '1OLDah-_p03Uv4hlUzQ1Pl');
            uuidB64 = uuidBase62.encode(uuid, {base: uuidBase62.baseX(customBase)});
            assert.equal(uuidB64, '1OLDah-_p03Uv4hlUzQ1Pl');

            var res = uuidBase62.decode(uuidB64, {base: customBase});
            assert.equal(res, uuid);
            res = uuidBase62.decode(uuidB64, {base: uuidBase62.baseX(customBase)});
            assert.equal(res, uuid);
            ...
            uuidB64 = uuidBase62.encode(uuid, {base: customBase});

            res = uuidBase62.decode(uuidB64, {base: customBase});
            assert.equal(res, uuid);
        });
